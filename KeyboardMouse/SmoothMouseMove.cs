namespace KeyboardMouse
{
    ...
    public static class SmoothMouseMove
    {

        static Random random = new Random();
        static int mouseSpeed = 15;

        public static void MoveMouseInCircle(double radius)
        {
            Point c = new Point();
            NativeMethods.GetCursorPos(out c);

            //get the center of the circle we want to draw
            int startX = c.X - (int)radius;
            int startY = c.Y;
            //iterate from 0 to 2PI (360 degrees)
            for (double i = 0.0d; i < Math.PI * random.NextDouble()*2; i += 0.1d)
            {
        ...
        public static void MoveMouse(Point pt, int rx =1, int ry = 1)
        {
            MoveMouse(pt.X, pt.Y,  rx, ry);
        }
